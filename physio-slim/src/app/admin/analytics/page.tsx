...
export default function AnalyticsAdminPage() {
  ...
  <div className="text-center text-gray-400 text-[13px] py-6">
    No leads yet
  </div>
) : (
  Object.entries(leadsByStatus).map(([status, count]) => {
    ...
    return (
      <div key={status} className="flex items-center justify-between">
        <span className="text-[12px] font-montserrat font-semibold capitalize text-gray-300">
          {status}
        </span>
        ...
        <div className="flex items-center gap-3">
          <div className="w-32 h-1.5 bg-[var(--border)]">
            <div
              ...
              style={{
                width: `${Math.round((countValue / leads.length) * 100)}%`,
              }}
            />
          </div>

          <span className="text-[13px] font-montserrat font-bold text-gold w-8 text-right">
            {countValue}
          </span>
        </div>
      </div>
